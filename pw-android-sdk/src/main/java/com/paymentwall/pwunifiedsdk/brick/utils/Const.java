...
public class Const {
	...
	public enum CardType {
		VISA(), MASTERCARD, AMEX, DISCOVER, JCB, DINERS_CLUB, UNIONPAY, UNKNOWN
	}
	...
	public static CardType getCardType(String cardNumber) {
		...
		if (trimmed.matches("^4[0-9]{12}(?:[0-9]{3})?$")) {
			return CardType.VISA;
		} else if (trimmed.matches("^5[1-5][0-9]{14}$") || trimmed.matches("^2(2[2-9][0-9]{12}|[3-6][0-9]{13}|7[0-1][0-9]{12}|720[0-9]{12})$")) {
			return CardType.MASTERCARD;
		} else if (trimmed.matches("^3[47][0-9]{13}$")) {
			return CardType.AMEX;
		} else if (trimmed.matches("^6(?:011|5[0-9]{2})[0-9]{12}$")) {
			return CardType.DISCOVER;
		} else if (trimmed.matches("^(?:2131|1800|35\\d{3})\\d{11}$")) {
			return CardType.JCB;
		} else if (trimmed.matches("^3(?:0[0-5]|[68][0-9])[0-9]{11}$")) {
			return CardType.DINERS_CLUB;
		} else if (trimmed.matches("^62[0-9]{14,17}$")) {
	...
	public static CardType getCardTypeFromName(String name) {
		if (name == null) return CardType.UNKNOWN;
